...
class FirestoreDoseRepository(
    ...
    override fun observeDay(uid: String, logicalDay: LocalDate): Flow<DataEnvelope<List<DoseState>>> =
        withWriteFailure(
            uid,
            callbackFlow {
                var lastValue = emptyList<DoseState>()
                val registration = FirestorePaths.doseStates(firestore, uid)
                    .whereEqualTo("logicalDay", logicalDay.toString())
                    .addSnapshotListener(MetadataChanges.INCLUDE) { snapshot, error ->
                        if (error != null) {
                            trySend(DataEnvelope(value = lastValue, fromCache = true, errorMessage = "Dose status could not be refreshed."))
                        ...
                        if (snapshot != null) {
                            ...
                            activeState.entries.removeIf { entry ->
                                entry.value.ownerUid == uid && entry.value.logicalDay == logicalDay && entry.key !in returnedKeys
                            }
    ...
    override fun observeHistory(uid: String): Flow<DataEnvelope<List<DoseEvent>>> =
        withWriteFailure(
            ...
            callbackFlow {
                ...
                val registration = FirestorePaths.doseEvents(firestore, uid)
                    .orderBy("syncedAt", Query.Direction.DESCENDING)
                    .limit(HISTORY_LIMIT)
                    .addSnapshotListener(MetadataChanges.INCLUDE) { snapshot, error ->
                        if (error != null) {
                            trySend(DataEnvelope(value = lastValue, fromCache = true, errorMessage = "History could not be refreshed."))
                            return@addSnapshotListener
                        }
                        if (snapshot != null) {
                            writeFailures.recordHealthySnapshot(uid)
    ...
    override suspend fun check(
        ...
        slot: DoseSlot,
        source: CheckSource,
    ): Boolean = checkWithMetadata(uid, logicalDay, medicine, slot, source, UUID.randomUUID().toString(), clock.instant())
    ...
    override suspend fun checkWithAction(
        ...
        actionId: String,
        occurredAt: Instant,
    ): Boolean = checkWithMetadata(uid, logicalDay, medicine, slot, source, actionId, occurredAt)
    ...
    private suspend fun checkWithMetadata(
        ...
        if (DoseActionPolicy.check(previous) != DoseCommandDecision.APPLY_CHECK) return@withLock false
    ...
    override suspend fun skip(
        ...
        val stateKey = actionKey(uid, stateId)
        val previous = activeState[stateKey]
        if (DoseActionPolicy.skip(previous) != DoseCommandDecision.APPLY_SKIP) return@withLock false
        val now = clock.instant()
        val zone = ZoneId.systemDefault().id
        val actionId = UUID.randomUUID().toString()
        val normalizedReason = reason.trim().take(MAX_SKIP_REASON_LENGTH).ifBlank { null }
        val state = DoseState(
            id = stateId,
            ownerUid = uid,
            logicalDay = logicalDay,
        ...
        activeState[stateKey] = state
        writeAction(state, previous, actionId, DoseAction.SKIP, now, zone, source)
        true
    }
    ...
    override suspend fun undo(
        ...
        if (DoseActionPolicy.undo(current, source) != DoseCommandDecision.APPLY_UNDO || current == null) return@withLock false
        val now = clock.instant()
        val zone = ZoneId.systemDefault().id
        val actionId = UUID.randomUUID().toString()
    ...
    private fun writeAction(
        ...
        batch.set(
            ...
            mapOf(
                ...
                "previousActionId" to if (action == DoseAction.UNDO) rollbackState?.lastActionId else null,
    ...
    private fun dispatchWrite(batch: WriteBatch, pendingWrite: PendingWrite) {
        try {
            batch.commit().addOnCompleteListener { completeWrite(pendingWrite, it.isSuccessful) }
    ...
    private fun completeWrite(pendingWrite: PendingWrite, successful: Boolean) {
        try {
            if (successful) {
                writeFailures.recordSuccess(pendingWrite.failureOperation)
                onWriteOutcome(pendingWrite.state.toWriteOutcome(pendingWrite.actionId, pendingWrite.action, true))
            } else {
                ...
                if (rollbackState == null) activeState.remove(stateKey, state) else activeState.replace(stateKey, state, rollbackState)
                onWriteOutcome(
    ...
    private fun <T> withWriteFailure(uid: String, source: Flow<DataEnvelope<T>>): Flow<DataEnvelope<T>> =
    ...
    private companion object {
        ...
        const val WRITE_FAILURE_MESSAGE = "A dose change could not be synchronised. Check your connection and try again."
        fun actionKey(uid: String, stateId: String): String = "$uid|$stateId"
    }
...
): DoseWriteOutcome = DoseWriteOutcome(
    ownerUid = ownerUid,
    actionId = actionId,
